...
import fireducks.pandas as pd  # Import FireDucks pandas
...
def save_to_excel(data, filename):
    # Create a DataFrame using FireDucks pandas
    df = pd.DataFrame(data, columns=["Student Name", "Email ID", "Assignments Turned In"])
    
    # Write the DataFrame to an Excel file
    df.to_excel(filename, sheet_name="Students", index=False, engine="xlsxwriter")
...
def main():
    ...
    # Save to Excel file
    save_choice = input("\nDo you want to save the data to an Excel file? (y/n): ").strip().lower()
    if save_choice == 'y':
        filename = f"{classroom_name}_students.xlsx"
        save_to_excel(student_data, filename)
